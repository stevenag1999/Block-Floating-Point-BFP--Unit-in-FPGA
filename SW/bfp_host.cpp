...
int main(int argc, char** argv) {
    ...
    auto bo_in_fp32_a = xrt::bo(device, size * sizeof(float), bfp_kernel.group_id(2));            // gmem0
    auto bo_in_exp_a  = xrt::bo(device, n_blocks * sizeof(unsigned int), bfp_kernel.group_id(3)); // gmem1
    auto bo_in_sign_a = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(4));     // gmem2
    auto bo_in_mant_a = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(5));     // gmem3
    
    // Input B buffers (BFP format for binary operations) - SHARE bundles with A
    auto bo_in_exp_b  = xrt::bo(device, n_blocks * sizeof(unsigned int), bfp_kernel.group_id(6)); // gmem1
    auto bo_in_sign_b = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(7));     // gmem2
    auto bo_in_mant_b = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(8));     // gmem3
    
    // Output buffers (FP32 for DECODE, BFP format for other operations) - SHARE bundles with inputs
    auto bo_out_fp32 = xrt::bo(device, size * sizeof(float), bfp_kernel.group_id(9));            // gmem0
    auto bo_out_exp  = xrt::bo(device, n_blocks * sizeof(unsigned int), bfp_kernel.group_id(10)); // gmem1
    auto bo_out_sign = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(11));     // gmem2
    auto bo_out_mant = xrt::bo(device, size * sizeof(unsigned int), bfp_kernel.group_id(12));     // gmem3
    ...
    float A1[N] = {
         ...
         8.0f, 9.0f, 1.11f, 2.0f, 3.3f, 1.55f, 2.15f, 2.16f
    };
    float B0[N] = {
